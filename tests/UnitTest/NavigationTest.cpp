...
namespace JsonNavigation
{
  ...
  "Object":{"Test1":"Test1", "Test2":"Test2"}
    ...
    TEST_F(NavigationTest, Position_Validation)
    {
        ...
        for (const auto& item : testPositions)
        {
            const auto& text = item.first;
            const auto& pos = item.second;
            EXPECT_GE(pos.nLine, static_cast<size_t>(0)) << "Invalid line number for: " << text;
            EXPECT_GE(pos.nColumn, static_cast<size_t>(0)) << "Invalid column for: " << text;
            EXPECT_GT(pos.nKeyLength, static_cast<size_t>(0)) << "Invalid key length for: " << text;
        }
