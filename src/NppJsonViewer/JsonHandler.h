...
public:
    ...
    auto ParseJson(const std::string &jsonText, rj::StringBuffer &sb, Handler &handler, SharedTrackingStream tss = nullptr) -> const Result;
...
inline auto JsonHandler::ParseJson(const std::string &jsonText, rj::StringBuffer &sb, Handler &handler, SharedTrackingStream tss) -> const Result
{
    Result retVal {};
    ...
    {
        success = tss ? reader.Parse<flgBase_Both>(*tss, handler) && sb.GetString() : reader.Parse<flgBase_Both>(*pSS, handler) && sb.GetString();
    }

    else if (!m_parseOptions.bIgnoreComment && m_parseOptions.bIgnoreTrailingComma)
    {
        success = tss ? reader.Parse<flgBase_comma>(*tss, handler) && sb.GetString() : reader.Parse<flgBase_comma>(*pSS, handler) && sb.GetString();
    }

    else if (m_parseOptions.bIgnoreComment && !m_parseOptions.bIgnoreTrailingComma)
    {
        success = tss ? reader.Parse<flgBase_comment>(*tss, handler) && sb.GetString() : reader.Parse<flgBase_comment>(*pSS, handler) && sb.GetString();
    }

    else if (!m_parseOptions.bIgnoreComment && !m_parseOptions.bIgnoreTrailingComma)
    {
        success = tss ? reader.Parse<flgBase>(*tss, handler) && sb.GetString() : reader.Parse<flgBase>(*pSS, handler) && sb.GetString();
    }
